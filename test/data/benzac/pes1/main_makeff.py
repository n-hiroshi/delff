...
def supercell(molname,supercell=[3,3,3]):
    gjffile=molname + '.gjf'
    LH=LammpsHandler()
    GCH=GaussianCartHandler()
    system0=GCH.getSystem(gjffile)
...
# (11)
def mq(molname,ffname,supercell=[1,1,1]):
    gjffile=molname + '.gjf'
    LH=LammpsHandler()
    GCH=GaussianCartHandler()
    system0=GCH.getSystem(gjffile)
    ...
    #:LH.run(system0,ffname=ffname,infile='npt.in',calcname=molname+'_cluster',test=False)
    LH.run(system0,ffname=ffname,infile='npt300K.in',calcname=molname+'_npt300K',test=False)
...
if __name__ == '__main__':
    ...
    if jobtype ==0:
        ...
        rotation_list_list=[list(range(-270,-95,5))]
    ...
    #(10) crystal opt
        ...
        ffname = sys.argv[3]
        mq(molname,ffname,supercell=[3,8,4])

    elif jobtype == 12:
        trjname = sys.argv[3]
        nmol = int(sys.argv[4])
        natom = int(sys.argv[5])
        trj2gjf(molname,trjname,nmollist=[nmol],natomlist=[natom])
        ...
        supercell(molname,supercell=[3,3,3])
