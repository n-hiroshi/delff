...
def main(idx,algo):
    ...
    # monomer
    in_settings_file = refdir + 'biphen.in.settings'
    data_file = refdir + 'biphen.data'
    monomer_gjf = refdir + 'biphen.gjf'
    atomtypesets_from1 = [[1,6],[2,3,11,12],[4,7,15,17],[9,19],[5,8,16,18],[10,13,20,21],[14,22]]
    atomtypesets = [[v-1 for v in list0] for list0 in atomtypesets_from1]

    # crystal lat
    lat_gjf  = refdir + 'biphen_lat.gjf'

    Hsubl    = 81.8 # https://webbook.nist.gov/cgi/cbook.cgi?ID=C92524&Mask=4 
    T = 383 #
    ...
    ff_ini, work = delff_lat.prepare(root,
                                     ...
                                     ewald=True,
                                     alpha_ewald= 0.13046973,
                                     nkmaxs=jnp.array([1,1,1])
                                     )
