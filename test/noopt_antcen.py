...
def main(idx,algo):
    ...
    # monomer
    in_settings_file = refdir + 'antcen.in.settings'
    data_file = refdir + 'antcen.data'
    monomer_gjf = refdir + 'antcen.gjf'
    atomtypesets_from1 = [[2,7],[1,3,4,9],[6,8,10,16],[11,14,17,21],[5,13],[12,15,18,22],[19,20,23,24]]
    atomtypesets = [[v-1 for v in list0] for list0 in atomtypesets_from1]

    # crystal lat
    lat_gjf  = refdir + 'antcen_lat211.gjf'

    Hsubl    = 100.2 #[kJ/mol] at 298.15K ref: McDonagh JCIM56(2016)2162
    T = 298.15
    ...
    ff_ini, work = delff_lat.prepare(root,
                                     ...
                                     ewald=True,
                                     alpha_ewald= 0.18228667,
                                     nkmaxs=jnp.array([2,1,1])
                                     )
    if algo in ['BO']:
